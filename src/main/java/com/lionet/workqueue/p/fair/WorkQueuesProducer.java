package com.lionet.workqueue.p.fair;
 ...
 * @Description rabbitmq 简单模式：一个生产者
 * <p>
 * 1. 获取连接
 * 2. 创建通道
 * 3. 声明队列
...
public class WorkQueuesProducer {
    ...
    public static void main(String[] args) throws Exception {
         ...
         */
        // channel.basicQos(1000, 1, false);
        //指该消费者接收到队列里的消息,但没有返回确认结果之前,队列不会将新的消息分发给该消费者。队列中没有被消费的消息不会被删除，还是存在于队列中。
        channel.basicQos(1);
        /**
         * @param queue 队列的名字
         *
         ...
         *
         * @param routingKey  路由key， 因为简单模式不存在交换机，此处设置为 队列； 路由key ，#匹配0个或多个单词，*匹配一个单词，在topic exchange做消息转发用
         *
        ...
        // 创建一个生产者
        for (int i = 0; i < 100; i++) {
            channel.basicPublish("", HELLOWORDQUEUENAME, null, (i + "").getBytes());
            Thread.sleep(150);
        }
